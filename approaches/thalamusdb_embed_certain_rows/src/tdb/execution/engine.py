...
class ExecutionEngine:
    ...
    def run(self, query, constraints):
        ...
        nr_certain_rows = 0
        embedding_order = False
        
        semantic_operators = self._create_operators(query)
        for operator in semantic_operators:
            operator.prepare()

        all_filters = [op for op in semantic_operators if isinstance(op, UnaryFilter)]
        image_filters = [f for f in all_filters if 'image_path' in f.filtered_column]
        ...
        while error > 0:
            ...
            if self._is_agg_results(results):
                print("_is_agg_results is executed")
                aggregate_results = AggregateResults(results)
            else:
                print("Arrive 1")
                aggregate_results = RetrievalResults(results)
                certain_rows = aggregate_results.intersection
                nr_certain_rows = len(certain_rows)
                if nr_certain_rows >= query.limit:
                    ...
                    break
                #Changed: 
                elif embedding_order == False and nr_certain_rows > 0:
                    print("Arrive 2")
                    if len(all_filters) != 0:
                        print("Arrive 3")
                        start_t_2 = time.time()
                        #Since images take the most time, first ordering on image is tried
                        # if no images, another filter is taken.
                        if len(image_filters) > 0:
                            used_filter = image_filters[0]
                        else:
                            used_filter = all_filters[0]
                        used_filter.create_embedding_order(certain_rows)
                        print('time startup embedding:', time.time() - start_t_2)
        ...
        end_time = time.time() - start_s
        return best_guess_result, counters, end_time