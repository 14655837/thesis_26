...
class SemanticOperator:
    ...
    def _get_description(self, item_text):
        ...
        # Find all tables that have both a _path column and a description column
        tables = [row[0] for row in self.db.execute2list("SHOW TABLES")]
        for table in tables:
            cols = [row[1] for row in self.db.execute2list(f"PRAGMA table_info('{table}')")]
            #path_col = next((c for c in cols if c.endswith("_path")), None)
            path_col = next((c for c in cols if c == "path" or c.endswith("_path")), None)
            if path_col is None or "description" not in cols:
                continue
            try:
                result = self.db.execute2list(
                    f"SELECT description FROM {table} "
                    f"WHERE {path_col} LIKE '%{filename}%' "
        ...
        if any(item_text.endswith(ext) for ext in ['.png', '.jpg', '.jpeg', '.wav', '.mp3']):
    ...
    def _encode_item(self, item_text):
        ...
        description = self._get_description(item_text)
        if description:
