...
class UnaryFilter(SemanticOperator):
    ...
    def _evaluate_predicate_parallel(self, item_texts, batch_size = 10):
        ...
        length = len(item_texts)
        num_iterations = (length + batch_size - 1) // batch_size
        ...
        for i in range(num_iterations):
            ...
            base = self._best_model_args(messages)['filter']
            kwargs = {**base, 'messages': messages}
            inputs.append((message_input, kwargs))
        ...
        for item_text, _, response in inputs_outputs:
            ...
            results.extend(self._extract_batch_response(item_text, result))            
    ...
    #ADDED func 
    def _batch_message(self, item_texts):
        """Creates a message from the LLM that describes the evaluation task
        ...
        for idx, item_text in enumerate(item_texts):
            # Label for this item
            content.append({'type': 'text', 'text': f'I{idx}:'})
            content.append(self._encode_item(item_text))

        return {'role': 'user', 'content': content}
    
    #ADDED func
    def _extract_batch_response(self, item_texts, response_text):
        """"""
        print(f"LLM response: {response_text}")
        print(f"Items in batch: {item_texts}")
        ...
        for token in response_text.split(','):
            ...
            if token.startswith('I'):
                try:
                    response_indices.append(int(token[1:]))
                except ValueError:
                    pass

        return [
            (item_text, idx in response_indices)
            for idx, item_text in enumerate(item_texts)
        ]
    ...
    def execute(self, order, remaining_needed = None):
